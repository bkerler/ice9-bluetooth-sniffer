...
struct SoapySDRDevice *soapysdr_setup(int id)
{
    ...
    for (size_t x=0;x<length;x++)
    {
        ...
        else if (device_samplerate>samp_rate)
        {
            if ((device_samplerate/samp_rate) < 1.3f)
            {
                samp_rate = (float)device_samplerate;
                break;
            }
            else {
                samp_rate = tmprate;
...
static int soapy_rx_cs8(const int8_t* buffer, size_t num_samples, float fullScale) {
    ...
    s->num = num_samples / 2;
    for (i = 0; i < s->num; ++i)
        s->samples[i] = buffer[2*i] / fullScale + buffer[2*i+1] / fullScale * I;
    if (running)
        push_samples(s);
    else
        free(s);
...
void *soapysdr_stream_thread(void *arg)
{
    ...
    complex float *samples=(complex float*)malloc(samples_per_buffer*sizeof(float complex));
    unsigned timeout=1000 * channels / 2 * 4096 / samp_rate;
        ...
        //receive some samples
    {
        ...
        if (ret>0)
        {
            ...
            rx_chain(buffs[0],ret*2, fullScale);
        }
        //else printf("U");
    }
    free(samples);
